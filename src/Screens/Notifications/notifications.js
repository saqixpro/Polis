...
const RenderCards = ({id, type, user, avatar, time, content, ondelete}) => {
  ...
  const leftSwipe = (progress, dragX) => {
    ...
    return (
      <View style={{ width: 100, backgroundColor: 'red', justifyContent: 'center', alignItems: 'center'}}>
        <Animated.Text style={{transform: [{scale: scale}], color: 'white', fontWeight: 'bold'}}>Delete</Animated.Text>
      </View>
    )
  }
     ...
     <Swipable renderRightActions={leftSwipe} onSwipeableRightOpen={ondelete} >
      ...
      <View style={styles.type}>
        <Image
          ...
          source={
            type == 'like'
              ? require('../../aseets/images/like.png')
              : type == 'comment'
              ? require('../../aseets/images/comment.png')
              : null
          }
        />
      </View>
